...
document.getElementById("create").addEventListener("click", () => {
    ...
    let input = document.getElementById("createORchange");
    let path = new String();
    if (selectElement) {
    ...
    if (input.value && path) {
        fetch(`/create?path=${btoa(path)}&content=${input.value}`)
            .then((response) => {
                return response.json();
            }).then((data) => {
                let id = data.response.id;
                if (id) {
                    let inAdd = undefined, array = new Array();
                    if (selectElement) {
...
document.getElementById("change").addEventListener("click", () => {
    ...
    if (input.value && id) {
        fetch(`/change?id=${id}&content=${input.value}`)
            .then((response) => {
                return response.json();
            }).then((data) => {
...
document.getElementById("remove").addEventListener("click", () => {
    ...
    if (deleteArray[0]) {
        fetch(`/remove?ids=${btoa(deleteArray.join())}`)
            .then((response) => {
                return response.json();
            }).then((data) => {
                console.log(data.response.info || data.response.status);
...
document.body.addEventListener('click', function (e) {
    ...
    if (!parsePath[0]) { clearSelectElement(); }
