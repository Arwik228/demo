...
document.getElementById("create").addEventListener("click", () => {
    ...
    if (input.value && path) {
        fetch(`/api/element/${input.value}`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ path: btoa(path) })
        }).then((response) => {
...
document.getElementById("change").addEventListener("click", () => {
    let input = document.getElementById("control_bar_input");
    let id = selectElement ? selectElement.dataset.id : false;
    if (input.value && id) {
        fetch(`/api/element/${input.value}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
...
document.getElementById("remove").addEventListener("click", () => {
    ...
    if (deleteArray[0]) {
        fetch(`/api/element/delete`, {
            method: 'DELETE',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ ids: btoa(deleteArray.join()) })
        }).then((response) => {
            return response.json();
        }).then((data) => {
